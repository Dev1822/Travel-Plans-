...
export const ExplorePage = () => {
  ...
  // Curated Stitch Editorial Presets
  ...
  return (
    <div className="bg-[#FCF9F8] text-[#1C1B1B] min-h-screen pb-32">
      ...
      <header className="w-full px-4 sm:px-8 lg:px-16 max-w-7xl mx-auto pt-32 pb-14 sm:pt-44 sm:pb-20 flex flex-col items-center text-center">
        <h1 className="font-serif text-4xl sm:text-6xl lg:text-7xl font-bold leading-tight mb-4">
          Explore India
        </h1>
        <p className="text-xs sm:text-base text-[#54433A] max-w-xl mb-10">
        ...
        >
          <Search className="w-5 h-5 absolute left-2 top-1/2 -translate-y-1/2 text-[#877369]" />
          <input
            type="text"
            value={localSearch}
            onChange={(e) => setLocalSearch(e.target.value)}
            placeholder="Search destinations, cities or states..."
            className="w-full bg-transparent border-b border-[#DAC2B6] focus:border-[#6C2F00] px-10 py-3 text-sm sm:text-base text-[#1C1B1B] placeholder-[#877369]/70 focus:outline-none transition-colors"
          />
        </form>
      </header>
      ...
      <section className="w-full px-4 sm:px-8 lg:px-16 max-w-7xl mx-auto pb-16 overflow-x-auto hide-scrollbar">
        <div className="flex gap-8 whitespace-nowrap border-b border-[#DAC2B6]/40 pb-4">
          {zones.map((z) => (
            <button
              key={z.id}
              type="button"
      ...
      {!searchQuery &&
        (activeZone === "all" ||
          ...
          <section className="w-full px-4 sm:px-8 lg:px-16 max-w-7xl mx-auto pb-24">
            <div className="relative w-full h-[500px] sm:h-[680px] overflow-hidden rounded bg-[#1C1B1B] group image-zoom cursor-pointer shadow-md">
              <div
              ...
              <div className="absolute bottom-0 left-0 p-8 sm:p-16 w-full md:w-3/4 text-white space-y-3">
                <span className="text-xs font-semibold uppercase tracking-widest text-[#FFB68C] block">
                  Featured Escape
                </span>
                <h2 className="font-serif text-3xl sm:text-5xl lg:text-6xl font-bold">
                  Udaipur, Rajasthan
                </h2>
                <p className="text-xs sm:text-sm text-[#FCF9F8]/90 max-w-xl leading-relaxed pt-1">
                  Lakes, palaces and timeless streets make Udaipur one of
                  India’s most atmospheric and serene escapes.
                </p>
                <div className="pt-4">
                  <Link
                    to="/destinations/udaipur"
                    className="inline-flex items-center space-x-2 text-xs font-semibold uppercase tracking-widest text-white border-b border-white pb-1 hover:opacity-80 transition-opacity"
                  >
      ...
      <section className="w-full px-4 sm:px-8 lg:px-16 max-w-7xl mx-auto space-y-12">
        <h3 className="font-serif text-2xl sm:text-4xl font-bold text-[#1C1B1B]">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-6 sm:gap-8">
            {/* Live Database Items + Editorial Curated */}
            {destinations.length > 0
              ? destinations.map((dest, idx) => {
                  const isLarge = idx % 5 === 0;
                  const spanClass = isLarge
                    ? "md:col-span-8 h-[460px]"
                    : "md:col-span-4 h-[460px]";
                  const cover =
                    dest.imageUrl ||
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="absolute bottom-0 left-0 p-6 sm:p-8 text-white">
                        <h4 className="font-serif text-2xl sm:text-4xl font-bold">
                          {dest.name}
                        </h4>
                        <p className="text-xs text-[#FFB68C] font-semibold uppercase tracking-wider mt-1">
